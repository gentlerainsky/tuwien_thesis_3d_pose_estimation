    ...
    def __init__(
        ...
        if intermediate:
            if mdn:
                ...
                self.intermediate_pred = nn.ModuleList(
                    [nn.Linear(16 * d_model, 16 * 3 * 5) for _ in range(n_layers)]
                )
                self.intermediate_enc = nn.ModuleList(
                    [nn.Linear(16 * 3 * 5, 16 * d_model) for _ in range(n_layers)]
                )
                self.intermediate_sigma = nn.ModuleList(
                    [nn.Linear(16 * d_model, 5) for _ in range(n_layers)]
                )
                self.intermediate_alpha = nn.ModuleList(
                    [nn.Linear(16 * d_model, 5) for _ in range(n_layers)]
                )
            else:
                self.intermediate_pred = nn.ModuleList(
                    [
                        nn.Sequential(
                            nn.Dropout(p=pred_dropout), nn.Linear(16 * d_model, 16 * 3)
                        )
                        for _ in range(n_layers)
                    ]
                )
                ...
                self.intermediate_enc = nn.ModuleList(
                    [nn.Linear(16 * 3, 16 * d_model) for _ in range(n_layers)]
                )
...
class LiftFormer(nn.Module):
    def __init__(
        ...
        if use_images:
            ...
            self.image_expand = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Conv2d(
                    feature_expansion // 2, feature_expansion, kernel_size=5, padding=0
                ),
                nn.ReLU(),
                nn.Conv2d(
                    feature_expansion, feature_expansion, kernel_size=3, padding=0
                ),
                nn.ReLU(),
            )
        else:
        ...
        if conv_enc:
            ...
            self.expand_conv = nn.Linear(
                j_nfeatures * num_joints_in, num_joints_in * feature_expansion
            )
        ...
        self.transformer_encoder = Transformer(
            ...
            dropout=dropout,
            intermediate=intermediate,
            mdn=mdn and intermediate,
            encoding=not spatial_encoding,
            pred_dropout=pred_dropout,
        )
        ...
        if not self.intermediate:
            # TODO: Add prediction dropout here as well?
            if self.mdn:
                self.decoder_mu = nn.Linear(
                    num_joints_in * t_nhid, num_joints_in * 3 * 5
                )
    ...
    def _weight_fuckery(self):
        ...
        for i in range(self.num_joints_in):
            enc[
                i * self.t_nhid : i * self.t_nhid + self.t_nhid, : i * self.j_nfeatures
            ] = 0
            enc[
                i * self.t_nhid : i * self.t_nhid + self.t_nhid,
                i * self.j_nfeatures + self.j_nfeatures :,
            ] = 0
        ...
        if self.intermediate:
            ...
            if self.mdn:
                (
                    ...
                    intermediate_sigma,
                    intermediate_alpha,
                ) = self.transformer_encoder(src, self.attn_mask)
                intermediate_mu = [out.view(b, j * 3, 5) for out in intermediate_mu]

                out = [
                    torch.cat([mu, sigma.unsqueeze(1), alpha.unsqueeze(1)], dim=1)
                    for mu, sigma, alpha in zip(
                        intermediate_mu, intermediate_sigma, intermediate_alpha
                    )
                ]
            else:
