...
class Graph():
    ...
    def get_distance_to_center(self,layout):
        ...
        elif layout == 'coco_upperbody':
            for i in range(self.seqlen):
                index_start = i*self.num_node_each
                dist_center[index_start+0 : index_start+7] = [0, 1, 1, 2, 2, 1, 1]
                dist_center[index_start+7 : index_start+11] = [2, 2, 3, 3]
                dist_center[index_start+11 : index_start+13] = [2, 2]
    ...
    def get_edge(self, layout):
        if layout == 'hm36_gt':
            ...
            sym_base = [(3, 4), (1, 2), (5, 6), (7, 8), (9, 10), (11, 12)]
            self_link, time_link = self.basic_layout(neighbour_base, sym_base)

            self.la, self.ra = [5, 7, 9], [6, 8, 10]
            # self.ll, self.rl = [12], [13]
            # self.cb = [0]
            # self.part = [self.la, self.ra, self.ll, self.rl, self.cb]
            self.part = [self.la, self.ra]
            self.edge = self_link + self.neighbour_link_all + self.sym_link_all + time_link
