...
for setup_name in ['all_actors']:
    for viewpoint in DRIVE_AND_ACT_VIEWPOINTS[-1:]:
        constructed_loader = get_drive_and_act_loaders(viewpoint, subset_percentage=100)
        for LitModel in ALL_LIGHTNING_MODELS[-1:]:
            ...
            labels = []
            for subset in subset_setup[setup_name][-1:]:
